...
namespace XRift.VrmExporter.Platforms.VRM.Baking
{
    /// <summary>
    /// lilToonマテリアルのテクスチャベイキング
    /// </summary>
    internal static class MaterialBaker
    {
        ...
        public static Texture? AutoBakeMainTexture(IAssetSaver assetSaver, Material material)
        {
            ...
            if (bakeSecond)
            {
                CopySecondTexProperties(hsvgMaterial, dictionaries);
            ...
            if (bakeThird)
            {
                CopyThirdTexProperties(hsvgMaterial, dictionaries);
        ...
        private static void CopySecondTexProperties(
            Material hsvgMaterial,
            (Dictionary<string, SerializedProperty> ts, Dictionary<string, SerializedProperty> fs,
                Dictionary<string, SerializedProperty> cs) dictionaries)
        {
        ...
        private static void CopyThirdTexProperties(
            Material hsvgMaterial,
            (Dictionary<string, SerializedProperty> ts, Dictionary<string, SerializedProperty> fs,
                Dictionary<string, SerializedProperty> cs) dictionaries)
        {
        ...
        private static Texture2D RunBake(Texture2D srcTexture, Material material, Texture2D? referenceTexture = null)
        {
        ...
        private static bool LoadTexture(IAssetSaver assetSaver, Texture? baseTexture, ref Texture2D srcTexture)
        {
            ...
            {
                UnityEngine.Object.Destroy(srcTexture);
        ...
        {
            if (dictionaries.ts.ContainsKey(key))
                material.SetTextureScale(key, dictionaries.ts[key].FindPropertyRelative("m_Scale").vector2Value);
        }
        ...
        {
            if (dictionaries.ts.ContainsKey(key))
                material.SetTextureOffset(key, dictionaries.ts[key].FindPropertyRelative("m_Offset").vector2Value);
        }
    }
}
